...
export default function AutocompleteSettings() {
  ...
  return (
    <div className="bg-aura-bg-elevated/50 backdrop-blur-md rounded-3xl p-6 lg:p-8 border border-white/5 shadow-2xl relative overflow-hidden group">
      ...
      <div className="space-y-6 relative z-10">
        <form onSubmit={handleAdd} className="flex gap-2">
          <input
            ...
            placeholder="e.g. Me bahar jaa raha hun"
          ...
          <button
            ...
            className="bg-primary text-background px-6 py-3 rounded-xl font-semibold hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
          >
            Add
          </button>
        </form>
          ...
          <AnimatePresence>
            {phrases.map((phrase) => (
              <motion.div
                ...
                className="flex items-center justify-between bg-black/20 border border-white/5 rounded-xl p-4"
              >
                <span className="text-aura-text-primary">{phrase}</span>
                <button
                  onClick={() => removePhrase(phrase)}
                  className="text-aura-danger/80 hover:text-aura-danger transition-colors p-2 rounded-lg hover:bg-aura-danger/10"
                  title="Remove phrase"
                >
