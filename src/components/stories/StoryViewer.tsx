  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[130] bg-[#0d0d15] font-sans text-[#e4e1ed] overflow-hidden"
      >
        {/* Playback Background Image/Text/Video */}
        <div className="absolute inset-0 z-0 bg-[#0d0d15] flex items-center justify-center">
          {loading ? (
            <div className="flex flex-col items-center gap-4">
              <div className="w-12 h-12 border-2 border-[#e6c487]/30 border-t-[#e6c487] rounded-full animate-spin"></div>
              <p className="font-label text-[10px] uppercase tracking-widest text-[#e6c487]/60">Decrypting Memory...</p>
            </div>
            ...
            <div className="px-12 text-center max-w-xl">
              <h1 className="text-3xl md:text-5xl font-serif italic text-[#e6c487] drop-shadow-2xl leading-tight">
        ...
        <header className="absolute top-0 left-0 w-full z-20 flex justify-between items-center px-6 py-4 pt-10 bg-gradient-to-b from-black/60 to-transparent">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full border-2 border-[#e6c487]/30 overflow-hidden shadow-xl">
               <img 
            ...
            <div className="flex flex-col">
              <span className="text-[#e6c487] font-serif italic text-sm tracking-wide drop-shadow-md">
            ...
            {stories.map((s, idx) => (
              <div key={s.id} className="h-[2px] flex-1 bg-white/20 rounded-full overflow-hidden">
                <motion.div 
                  ...
                  className="h-full bg-[#e6c487] rounded-full shadow-[0_0_8px_rgba(230,196,135,0.5)]"
                />
              </div>
            ))}
          </div>

          <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="text-[#e6c487] bg-black/20 backdrop-blur-md rounded-full p-2 hover:bg-black/40 transition-all active:scale-95">
