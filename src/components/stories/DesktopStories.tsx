...
export default function DesktopStories({ stories, onStoryClick, onAddStory }: DesktopStoriesProps) {
  ...
  return (
    <div className="h-screen overflow-y-auto bg-[var(--bg-primary)] text-[#e4e1ed] font-sans selection:bg-[rgba(var(--primary-rgb),_0.3)]">
      ...
      <header className="flex justify-between items-center px-12 h-24 sticky top-0 bg-[var(--bg-primary)]/80 backdrop-blur-xl z-40 border-b border-white/5">
        <div className="relative w-96">
          ...
          <input 
            ...
            className="w-full bg-[var(--bg-elevated)] border border-white/5 rounded-full py-3 pl-12 pr-6 text-sm focus:ring-1 focus:ring-[var(--gold)] focus:bg-[#292932] outline-none transition-all duration-300 placeholder:text-[#998f81]/40 text-white" 
          />
        </div>
      ...
      <div className="px-12 pb-20 max-w-7xl mx-auto">
        ...
        <section className="mb-12 mt-12">
          <h1 className="text-5xl font-serif italic font-bold text-[var(--gold)] mb-2 tracking-tight">Stories</h1>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {stories.map(story => (
            <div 
              ...
              className="relative group aspect-[3/4] rounded-[2.5rem] overflow-hidden bg-[var(--bg-elevated)] cursor-pointer transition-all duration-500 hover:scale-[1.02] hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.6)] border border-white/5 flex items-center justify-center flex-col"
            >
              {!story.media_url ? (
                <>
              ...
              <div className="absolute bottom-0 inset-x-0 p-6 z-10">
                <div className="bg-black/40 backdrop-blur-xl rounded-3xl p-5 flex items-center justify-between border border-white/10">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[rgba(var(--primary-rgb),_0.3)] shadow-xl flex items-center justify-center bg-[var(--bg-elevated)]">
                      {story.is_mine ? (
                  ...
                  {!story.is_mine && !story.viewed_at && (
                    <div className="w-3 h-3 bg-[var(--gold)] rounded-full shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)] mr-2"></div>
                  )}
                </div>
              </div>
            </div>
          ...
          {stories.length === 0 && (
            <div className="col-span-full text-center py-20 text-[#998f81]/50 font-label text-sm uppercase tracking-widest border border-white/5 rounded-3xl bg-[var(--bg-elevated)]/30 backdrop-blur-xl">
        ...
        <section className="bg-[var(--bg-elevated)]/30 backdrop-blur-md border border-white/5 rounded-[4rem] p-16 mb-20 relative overflow-hidden group shadow-3xl">
          <div className="absolute -right-20 -top-20 w-80 h-80 bg-[rgba(var(--primary-rgb),_0.05)] rounded-full blur-[80px] group-hover:bg-[rgba(var(--primary-rgb),_0.1)] transition-colors duration-700"></div>
          
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div>
              <h2 className="text-5xl font-serif italic font-bold text-[var(--gold)] mb-3">Create a Moment</h2>
              <p className="text-[#998f81] text-xl font-light italic">Capture a fragment of your day for only them to see.</p>
            </div>
            <button className="px-8 py-4 border border-[#998f81]/20 rounded-full text-[#998f81] hover:text-[var(--gold)] hover:border-[var(--gold)] transition-all text-[11px] font-bold tracking-widest uppercase active:scale-95 duration-300">
              View Archive
            </button>
          </div>
